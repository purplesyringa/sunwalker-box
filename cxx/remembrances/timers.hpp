...
struct State {
    std::array<int, 64> indices;
    std::array<int, 64> clock_ids;
    std::array<sigevent, 64> timers;
    std::array<itimerspec, 64> intervals;
    size_t count;
};
...
Result<void> add_timer(const int timer_id, const int clock_id, const sigevent &sev) {
    static int actual_timer_id;
    ...
    libc::timer_create(clock_id, const_cast<sigevent *>(&sev), &actual_timer_id)
        .CONTEXT("Could not create timer")
        .TRY();
    ENSURE(timer_id == actual_timer_id, "Unexpected timer ID");
    return {};
}

Result<void> add_dummy_timer(const int next_timer_id) {
    static sigevent dummy{.sigev_notify = SIGEV_NONE};
    return add_timer(next_timer_id, CLOCK_REALTIME, dummy).CONTEXT("Could not add dummy timer");
}
...
Result<void> load(const State &state) {
    ...
    for (int i = 0; i < state.count; ++i) {
        while (next_timer_id < state.indices[i]) {
            ...
            libc::timer_delete(next_timer_id).CONTEXT("While dummy destroying").TRY();
            next_timer_id += 1;
        }

        add_timer(state.indices[i], state.clock_ids[i], state.timers[i])
            .CONTEXT("While timer creating")
            .TRY();
        libc::timer_settime(state.indices[i], 0, &state.intervals[i], nullptr)
            .CONTEXT("While arming timer")
            .TRY();
        next_timer_id += 1;
    }
