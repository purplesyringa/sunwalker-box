...
Result<uintptr_t> guess_task_size() {
    ...
    uintptr_t right = uintptr_t{1} << (63 - 12);
    while (right - left > 1) {
...
Result<void> run() {
    ...
    libc::munmap(&end_of_bss, task_size - reinterpret_cast<uintptr_t>(&end_of_bss))
    ...
    libc::mmap(&start_of_text, stemcell_size, PROT_READ | PROT_WRITE | PROT_EXEC,
...
Result<void> init_child(const ControlMessageFds &fds) {
    ...
    rseq::load(state.rseq).CONTEXT("Failed to load rseq").TRY();

    while (!state.allowed_to_munmap_self) {
        // FIXME: This should #include <linux/futex.h>, but that header declares robust_list as a
        // struct, and we use the same name for a namespace--bummer
        const int FUTEX_WAIT = 0;
        libc::futex(&state.allowed_to_munmap_self, FUTEX_WAIT, 0, nullptr)
...
#error Trying to compile stemcell against unsupported architecture!
    ...
    libc::munmap(&start_of_text, stemcell_size).CONTEXT("Failed to munmap stemcell").TRY();
    __builtin_trap();
...
Result<void> loop() {
    ...
    for (;;) {
        ...
        (void)libc::mmap(&start_of_text, stemcell_size, PROT_READ | PROT_WRITE | PROT_EXEC,
                         MAP_FIXED | MAP_SHARED, memfd, 0);
