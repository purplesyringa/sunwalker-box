...
Result<void> run() {
    ...
    robust_list::save(state.robust_list).CONTEXT("Failed to save robust list").TRY();
    ...
    // This 24 is actually sizeof(robust_list_head); we don't #include <linux/futex.h> because that
    // makes robust_list ns coincide with struct
    libc::set_robust_list(nullptr, 24).CONTEXT("Failed to reset robust list").TRY();
