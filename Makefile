...
target/parasite.result_context_map.json: target/parasite.result_context_map generate_context_map.py
	python3 generate_context_map.py <$< >$@
...
target/stemcell.result_context_map: target/stemcell
	objcopy -O binary --only-section=.result_context_map $< $@
target/stemcell.result_context_map.json: target/stemcell.result_context_map generate_context_map.py
	python3 generate_context_map.py <$< >$@
target/stemcell.relocations: target/stemcell.stripped generate_stemcell_relocations.py
	python3 generate_stemcell_relocations.py $< >$@
