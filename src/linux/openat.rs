use crossmist::{Deserializer, NonTrivialObject, Serializer};
use std::io::Result;
use std::ops::Deref;
...
unsafe impl NonTrivialObject for OpenAtDir {
    ...
    unsafe fn deserialize_self_non_trivial(d: &mut Deserializer) -> Result<Self> {
        Ok(Self(unsafe {
            openat::Dir::from_raw_fd(d.deserialize::<OwnedFd>()?.into_raw_fd())
        }))
    }
}
