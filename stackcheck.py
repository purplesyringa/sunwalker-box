...
while True:
    ...
    if iteration == len(transitions) + 1:
        print("Cycle in allocations:", file=sys.stderr)
        address = updated_address
        while True:
            print(line_at_address[address], file=sys.stderr)
...
for symbol in sys.argv[1:]:
    ...
    stack_usage = -lowest.get(address, 0)
    print(stack_usage)

    print("Nesting leading to this usage:", file=sys.stderr)
    while True:
        if address in lowest_next:
            target, stack_manip = lowest_next[address]
            if stack_manip > 0:
                print(f"[+{stack_manip}]".ljust(10) + line_at_address[address], file=sys.stderr)
            elif stack_manip < 0:
                print(f"[{stack_manip}]".ljust(10) + line_at_address[address], file=sys.stderr)
            else:
                print(" " * 10 + line_at_address[address], file=sys.stderr)
            address = target
        else:
            print(" " * 10 + line_at_address[address], file=sys.stderr)
            break
