...
pub fn checkpoint() -> Result<()> {
    let arch_prctl_options = remembrances::arch_prctl_options::in_orig().context("Failed to save arch_prctl options")?;
    let itimers = remembrances::itimers::in_orig().context("Failed to save interval timers")?;
    let mm_options = remembrances::mm_options::in_orig().context("Failed to save memory map options")?;
    ...
    let thp_options = remembrances::thp_options::in_orig().context("Failed to save transparent huge pages options")?;
    ...
    remembrances::arch_prctl_options::in_master(arch_prctl_options).context("Failed to restore arch_prctl options")?;
    ...
    remembrances::mm_options::in_master(mm_options).context("Failed to restore memory map options")?;
    ...
    remembrances::thp_options::in_master(thp_options).context("Failed to restore transparent huge pages options")?;
